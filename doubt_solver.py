...
def create_chain(db):
    ...
    If you don't know the answer or if answer is not related to context, don't provide an answer, just say out of your scope of knowledge. \
...
def main():
    ...
    st.set_page_config(
        page_title="MentorAI: Your Personal DoubtSolver⚡",
        page_icon="📄",
        layout="centered",
    ...
    with st.sidebar:
        ...
        modes = ['Data Structures & Algorithms', 'Computer Networks & Security', 'Deep Learning']
        ...
        mode = st.sidebar.selectbox('Subject', modes, key='st.session_state.mode')
        ...
        if st.session_state.mode == modes[0]:
            # flag = 0
            print("Mode - Data Structures & Algorithms")
            vstore = "vector_db/faiss_index_1"
            welcome_message = "Ask me questions about Data Structures & Algorithms!"
        ...
        if st.session_state.mode == modes[1]:
            # flag = 0
            print("Mode - Computer Networks & Security")
            vstore = "vector_db/faiss_index_2"
            welcome_message = "Ask me anything about the world of Security!"
            if st.session_state.previous_mode != st.session_state.mode:
                st.session_state.init = False
                st.session_state.pop('messages', None)
        ...
        if st.session_state.mode == modes[2]:
            # flag = 0
            print("Mode - Deep Learning")
            vstore = "vector_db/faiss_index_3"
            welcome_message = "Ask me about Deep Learning!" 
            if st.session_state.previous_mode != st.session_state.mode:
                st.session_state.init = False
                st.session_state.pop('messages', None)          
        ...
        st.markdown("---")
        st.markdown("### About")
        st.info("MentorAI is your go-to digital mentor, offering expert solutions across all subjects. With specialized bots for each field, it provides quick, accurate answers to your academic doubts, helping you learn with confidence.")
    
    st.title("Welcome to MentorAI 🤖⚡")
    st.info("A mentor for every subject, a solution for every doubt.")
